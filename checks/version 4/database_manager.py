...
def set_auth_level(guild_id, user_id, level):
    ...
    # Remove user from both lists to ensure they don't have double levels
    auth_db[gid]["auth1"] = [user for user in auth_db[gid]["auth1"] if str(user) != uid]
    auth_db[gid]["auth2"] = [user for user in auth_db[gid]["auth2"] if str(user) != uid]
    
    if level == 1:
        auth_db[gid]["auth1"].append(uid)
    elif level == 2:
        auth_db[gid]["auth2"].append(uid)
